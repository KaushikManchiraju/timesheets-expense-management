...
public class ValidationTools {
    ...
    public ResultAndCause validateTimeEntry(TimeEntry timeEntry) {
        ...
        if (baseValidation.isPositive) {
            if (timeEntry.getTimeInMinutes() == null || timeEntry.getTimeInMinutes() == 0) {
                return ResultAndCause.negative(messages.getMessage(getClass(), "notification.emptyAmount"));
            }
        } else {
    ...
    public ResultAndCause validateExpenses(Expenses expenses) {
        ResultAndCause baseValidation = validateExpensesBase(expenses);
        if (baseValidation.isPositive) {
    ...
    protected ResultAndCause validateExpensesBase(ExpensesBase expenses) {
        if (expenses.getTask() == null) {
            return ResultAndCause.negative(messages.getMessage(getClass(), "notification.emptyTask"));
        }

        List<ActivityType> activityTypes = projectsService.getActivityTypesForProject(expenses.getTask().getProject(), View.MINIMAL);
        if (CollectionUtils.isNotEmpty(activityTypes) && expenses.getActivityType() == null) {
            return ResultAndCause.negative(messages.getMessage(getClass(), "notification.emptyActivityType"));
        }
    ...
    public ResultAndCause validateTags(ExpensesBase expenses) {
        Preconditions.checkNotNullArgument(expenses);
        Preconditions.checkNotNullArgument(expenses.getTask());
        Preconditions.checkNotNullArgument(expenses.getTask().getRequiredTagTypes());

        HashSet<TagType> remainingRequiredTagTypes = new HashSet<>(expenses.getTask().getRequiredTagTypes());
        if (CollectionUtils.isNotEmpty(expenses.getTags())) {
